...
# with timeout(time_out):
    ...
    # img_binary, _  = adaptive_masking(img_mcherry, mm_th = 3.5, th_sel = 0.3, exp_size = 3)
    img_binary, _  = adaptive_masking(img_mcherry, mm_th = 2, th_sel = 0.3, exp_size = 3, sorting = True)

    status = saving_log(debugpath, 'RUNNING', current_res['Frame'], current_res['Position'], 'Masked', runtime = toc(start0, False))
    ...
    status = saving_log(debugpath, 'RUNNING', current_res['Frame'], current_res['Position'], 'Labelled', runtime = toc(start0, False))
    ...
    values_quant = np.linspace(0,1,51).round(2)
    quant_vals = np.quantile(img_gfp[selected],q=values_quant)
    quant_vals = dict(zip(values_quant, quant_vals))
    current_res = {**current_res, **quant_vals}
    ...
    current_res['volume_sp'] = np.sum(cropped_binary)
