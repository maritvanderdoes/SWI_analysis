...
goodworms=['goodworms_LIN14_LIN29.csv','goodworms_LIN28.csv',"goodworms_HBL1.csv",'goodworms_LIN41.csv','goodworms_LIN42AB.csv']
results=['Results_lin14_lin29_V3.csv','Results_lin28_V3.csv',"Results_HBL1_V3.csv",'Results_LIN41_V3.csv','Results_lin42AB_V3.csv']
...
for i in range(np.size(goodworms)):
    ...
    #normalize and apply regresson on control
    control=CSV_quantification[CSV_quantification["Condition"]=='control'] #get control worms to fit data on
    Regression_results=control.groupby('Time').apply(regress, 'mean_curved', ['mean_background']) #do linear regression per timepoint
...
tp=20
controls_tp=control[control["Time"]==tp]
...
sns.lineplot(data=Samples[Samples["Condition"]=="HBL-1"],x="Time",y="signal",hue="old_condition").set_title("control_experiments together")
