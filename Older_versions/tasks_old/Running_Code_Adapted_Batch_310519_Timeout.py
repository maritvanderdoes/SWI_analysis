...
# Time out
from utils import timeout, raise_timeout
...
def main_function(list_mcherry,list_GFP):
    with timeout(time_out):
        ...
        try:
            ...
            for it in range(0,1):
                ...
                img_binary, additional_outputs  = \
                    adaptive_masking(img_mcherry, exp_size = 3)
                ...
                with open(foldername+'/RUNNING_Sample'+'_t'+current_res['Frame']+'_s'+current_res['Position']+'_Status_batch_'+str(batch_sel)+'.txt', 'w') as f:
                    f.write(status)
            ...
            stop_error = toc(start0)
            status = status+' (after error)'
            # os.remove(foldername+'/RUNNING_Sample'+'_t'+current_res['Frame']+'_s'+current_res['Position']+'_Status.txt')
            os.remove(foldername+'/RUNNING_Sample'+'_t'+current_res['Frame']+'_s'+current_res['Position']+'_Status_batch_'+str(batch_sel)+'.txt')
            # Saving status
            # with open(foldername+'/ERROR_Sample'+'_t'+current_res['Frame']+'_s'+current_res['Position']+'_Status.txt', 'w') as f:
            with open(foldername+'/ERROR_Sample'+'_t'+current_res['Frame']+'_s'+current_res['Position']+'_Status_batch_'+str(batch_sel)+'.txt', 'w') as f:
                f.write(status + ', with time of '+ str(np.round(stop_error,2)) + ' (seconds)')
        ...
        if debugging:
            ...
            with open(foldername+'/COMPLETED_Sample'+'_t'+current_res['Frame']+'_s'+current_res['Position']+'_Status_batch_'+str(batch_sel)+'.txt', 'w') as f:
...
# pip install -e .
#
# python tasks/Running_Code_Adapted_Batch_310519_Timeout.py
