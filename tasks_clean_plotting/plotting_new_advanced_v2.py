...
goodworms=['goodworms_LIN14_LIN29.csv','goodworms_LIN42AB.csv', 'goodworms_LIN29_NHR23i.csv']
results=['Results_lin14_lin29_V3.csv','Results_lin42AB_V3.csv', 'Results_NRH23i.csv']
...
median_volumes = CSV_sing.reset_index().groupby(['old_condition','Time']).max()

th = 0.8
sample = 97
chamber_idx_vec = pd.unique(CSV_sing.index)
chamber_volume = CSV_sing.loc[chamber_idx_vec[sample],['Time','volume_curved']].reset_index().groupby('Time').mean()
...
CSV_sing.loc[chamber_idx_vec[sample],'actual_volume'] = chamber_volume['actual_volume']
...
median_volumes = CSV_sing.reset_index().groupby(['old_condition','Time']).max()
CSV_processed = CSV_sing.copy()
th = 0.8
sample = 97
chamber_idx_vec = pd.unique(CSV_sing.index)
for sample, chamber_idx in enumerate(pd.unique(CSV_sing.index)):
    chamber_volume = CSV_sing.loc[chamber_idx,['Time','volume_curved','mean_curved', 'mean_background']].reset_index().groupby('Time').mean()
...
#%%
dynamics_to_plot = CSV_processed.reset_index().groupby(['old_condition','Time']).mean().reset_index()
# dynamics_to_plot = CSV_processed.reset_index().groupby(['old_condition','Time','Position']).mean().reset_index()
sns.lineplot(data = dynamics_to_plot, x = 'Time', y = 'mean_curved', hue = 'old_condition')

nhr23_df = dynamics_to_plot.loc[dynamics_to_plot['old_condition'].isin(['LIN29_GFP_smpl','LIN42_GFP_smpl','control_smpl'])]
# nhr23_df = dynamics_to_plot.loc[np.invert(dynamics_to_plot['old_condition'].isin(['LIN29_GFP_smpl','LIN42_GFP_smpl','control_smpl']))]
nhr23_df = nhr23_df[['old_condition','Time','Position','mean_curved','mean_background','volume_curved']].dropna()
plt.figure()
sns.lineplot(data = nhr23_df, x = 'Time', y = 'mean_curved', hue = 'old_condition')

plt.figure()
sns.lineplot(data = nhr23_df, x = 'Time', y = 'mean_background', hue = 'old_condition')

#%%
nhr23_df['QuickComp'] = nhr23_df['mean_curved']-nhr23_df['mean_background']
sns.lineplot(data = nhr23_df, x = 'Time', y = 'QuickComp', hue = 'old_condition')
...
reference_conditon =  'control_14_29'
reference_conditon =  'control_lin42ab'
...
#%%
sns.lineplot(data = Regression_results.reset_index(), x= 'Time', y = 'signal', hue = 'old_condition')
