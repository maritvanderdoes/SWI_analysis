...
# debugpath       = dirsel + '/Debug'
# filename        = 'Worm_segmentation_results_20211210_lin29GFP_nhr23i.csv'
...
def folder_documentation(list_file):
    ...
    if data_format == 'st':
        ...
        (frame_set, rest) = rest.split('.stk')
    elif data_format == 'ts':
        (basename, rest) = filename.split('_t')
        (frame_set, rest) = rest.split('_s')
        (position_set, rest) = rest.split('.stk')
        ...
        for prob_set in range(0,min(estimates.shape[0],n_plots_th)):
            ...
            for k_t, t_set in enumerate([t_sel-1,t_sel,t_sel+1]):
                if (t_set>0)&(t_set<=subset['Frame'].max()):
                    ...
                    mat_sel = img_mcherry.max(axis = 2).max(axis = 1)
                    axs_image.imshow(img_mcherry[mat_sel.argmax(), :, :], cmap = 'gray')
