...
def main_function(list_mcherry,list_GFP):
    ...
    try:
        ...
        cropped_binary, cropped_image = crop_image(img_binary, img_gfp, verbose = verbosity)
        status = 'Cropped Image'

        # Calculating curved worms properties 
        (mean_curved, volume_curved) = calculate_worm_properties(cropped_binary, cropped_image, verbose = verbosity)
        ...
        straightened_image, (xcoord, ycoord)  = straighten_image2D(max_image,  X, Y, dx, dy, verbose = verbosity)
        straightened_binary, (xcoord, ycoord) = straighten_image2D(max_binary, X, Y, dx, dy, verbose = verbosity)
