...
sngpln = False  # Checking a single plane (similar to ben's code)
    ...
    with timeout(1):
        ...
        try:
            ...
            for it in range(0,1):
                ...
                # Spline fitting
                X, Y, dx, dy = create_spline(Xinput, Yinput, s=200, verbose = verbosity)
...
if parallel:
    ...
    if __name__=='__main__':
        ...
        #     p = multiprocessing.Process(target=main_function, args=zip(list_mcherry,list_GFP), name = ('process_' + str(i+1)))
        #     procs.append(p)
        #     p.start()
        ...
        # for p in procs:
        #     p.join(80)
        #     print('Timeout', p.name)

        #p.starmap(TimeoutError)
...
#
#
# python tasks/Development_with_data_Lucas_parallel_timeout.py
