...
for position_sel in iterating_vec[0:]:
    ...
    axs_growth.set_title('Sample: '+str(position_sel)+', Condition: '+subset.loc[subset['Frame']==t_sel,'Condition'].values[0]+\
        ', NAs: '+str(selection_csv.isnull().sum()['mask_mean'])+ ', mean_ecc: '+str(np.round(selection_csv['mask_eccentricity'].mean(),2)),
        fontsize = 20)



    fig.savefig(dirsel+'/worm_detect/'+basename+'_s'+str(position_sel)+'_Full_analysis'+'.pdf', dpi = 200)
    plt.close()
