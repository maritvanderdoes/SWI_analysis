...
if parallel:        
    ...
    n_batches = 1#int(np.ceil(n_samples/batch_size))
    start_parallel = tic()
    ...
    for batch_sel in np.arange(0,n_batches):
        ...
        df.to_csv(outputpath+'/Results_batch_2_sorted_straightened_'+str(batch_sel)+'_'+str(n_batches)+'.csv', index = False)
    ...
    # Saving files
    df = pd.DataFrame(results)
    df.to_csv(outputpath+'/Results_debugging_with_hist_quant.csv', index = False)
