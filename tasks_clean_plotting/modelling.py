...
RNA["Time_scaled"]=RNA["Time"]*1.2273 - 2.3643
...
def rna2protein_model(p,t): 
    m_t = m[int(np.floor(t))]
    dpdt = alpha*m_t-beta*p
    return dpdt

name = 'HBL-1'
# name = 'LIN42BC'
tm = pd.unique(RNA["Time_scaled"])
m = 2**RNA[RNA["Name"]==name]['value'].values

p_ref = Samples[Samples['Condition']==name].groupby('Time').mean().sort_values(by = 'Time')
...
dt = .1;
TMAX = RNA["Time"].max()-20*dt;


tp = np.arange(0,TMAX,dt)+1

# Integration
ps = odeint(rna2protein_model, p0, tp)
# tp = tp*scal_factor
tp=tp*1.2273 - 2.3643

slop_scal = 1
min_scal = 0
ps_norm = (ps-ps.min())/(ps.max()-ps.min())*slop_scal+min_scal

plt.plot(tm, m, 'k')
plt.plot(tp, ps,'r')
plt.legend(['RNA', 'Predicted Protein'])
plt.xlabel('Time')
plt.ylabel('Levels')
plt.title(name+', $\\beta=$'+str(beta))
plt.savefig(name+'_beta_'+str(beta)+"_pred.pdf", dpi = 200)

#%

tp = tp*1

pref = p_ref['signal'].values
pref[pref<0] = 0
pref_norm = (pref-pref.min())/(pref.max()-pref.min())

plt.figure()
plt.plot(p_ref.index, pref_norm)
plt.plot(tp, ps_norm,'r')
plt.legend(['Measured protein', 'Predicted Protein'])
plt.xlabel('Time')
plt.ylabel('Normalised Levels')
plt.title(name+', $\\beta=$'+str(beta))
plt.savefig(name+'_beta_'+str(beta)+"_comp.pdf", dpi = 200)
...
plt.xlabel('Time')
plt.ylabel('Normalised Levels')
plt.title(name+', $\\beta=$'+str(beta)+', $n_t=$'+str(tmin)+', $m_p =$'+str(psp)+', $n_p=$'+str(pmin))
plt.savefig(name+'_beta_'+str(beta)+"_comp_manual_scale.pdf", dpi = 200)
...
plt.ylabel('Normalised Levels')
plt.title(name+', $\\beta=$'+str(beta))
plt.savefig(name+'_beta_'+str(beta)+"_comp_alt_t0.pdf", dpi = 200)
...
plt.ylabel('Normalised Levels')
plt.title(name+', $\\beta=$'+str(beta)+', $n_t=$'+str(tmin)+', $m_p =$'+str(psp)+', $n_p=$'+str(pmin))
plt.savefig(name+'_beta_'+str(beta)+"_comp_alt_t0_manual_scale.pdf", dpi = 200)
...
means = controls_tp.groupby('Condition').mean()

x_fit = np.linspace(np.min(controls_tp["mean_background"])-10,np.max(controls_tp["mean_background"])+10,100)
y_fit = regression_multiple["bg_factor"].values[None,:]*x_fit[:,None] + regression_multiple["internal_noise"].values[None,:] +  regression_multiple["signal"].values[None,:]
plt.plot(x_fit, y_fit)

# m_val =  regression_multiple["bg_factor"]['HBL-1']*means['mean_background']['control']+ regression_multiple["internal_noise"]['HBL-1'] +  regression_multiple["signal"]['HBL-1']
# plt.plot([x_fit.min(),means['mean_background']['control']],[m_val, m_val],'r:')
# plt.plot([means['mean_background']['control'],means['mean_background']['control']],[y_fit.min(), m_val],'r:')

plt.plot([x_fit.min()*np.ones(2),means['mean_background'].values],[means['mean_curved'].values,means['mean_curved'].values],'k:')
plt.plot([means['mean_background'].values,means['mean_background'].values],[y_fit.min()*np.ones(2),means['mean_curved'].values],'k:')


plt.savefig('Figure_HBL1_20_v2.pdf', dpi = 200)
